...
def AI_prefrop(AI_hand,bet_count,emotion):
  print('bet_count: '+str(bet_count))
  #emotion
  if emotion == "Angry" or emotion == "Disgust" or emotion == "Sad" :
    return 'bet to '
  elif  emotion == "Fear" or emotion == "Happy" or emotion == "Surprise":
    #premium
    ...
    return defult_prefrop(AI_hand,bet_count)
def defult_prefrop(AI_hand,bet_count):
  #premium
  if (int((AI_hand[0])[1:]) > 9 or int((AI_hand[0])[1:]) == 1) and (int((AI_hand[1])[1:]) >9 or int((AI_hand[1])[1:]) == 1):
    if bet_count < 3 :
      return 'bet to '
...
def AI_action(AI_hand,bord,action_count,bet_count,emotion):
  index = AI_hand + bord
  qualify_alredy = ''
  ...
  if emotion == "Angry" or emotion == "Disgust" or emotion == "Sad" :
    ...
    return defult_post(stronger_qualify,action_count,bet_count)

def defult_post(stronger_qualify,action_count,bet_count):
  if stronger_qualify == 'StraightFlush':
      if bet_count < 5:
        return 'bet to '
      elif bet_count > 0:
    ...
    for x in itertools.combinations(index, 4):
      if judge_flush_draw(x) and action_count < 3 :
        if bet_count > 0:
          return 'call'
        else:
