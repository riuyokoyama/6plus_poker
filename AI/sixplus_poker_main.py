...
def call_action():
    global ai_bet,player_chip,player_chip_label,pot,bet_count,game_pot,bet_action_count,ai_chip,ai_label,player_bet,bet_count,turn_message
    turn_message.set('AI Turn')
    bet_action_count += 1
...
def check_action():
    global check_button,bet_action_count,bet_count,ai_bet,turn_message,player_bet,pot,ai_chip,player_chip
    turn_message.set('AI Turn')
    emotion = take_face_emotion.start_get_emotion()
    print('1st bet_count: '+str(bet_count))
    bet_action_count += 1
    if bet_action_count == 1:
        AI_action = poker_AI.AI_prefrop(ai_hands_index,0,emotion)
        if AI_action == 'bet to ':
            turn_message.set('Player Turn')
            bet_action_count -= 1
            bet_count += 1
        ...
        AI_action = poker_AI.AI_action(ai_hands_index,board_index[:(bet_action_count+2)],bet_action_count,bet_count,emotion)
        if AI_action == 'bet to ':
            turn_message.set('Player Turn')
            bet_action_count -= 1
            bet_count += 1
...
def bet_action():
    global game_pot,pot,player_chip,ai_chip,bet_value,bet_button,fold_button,bet_action_count,bet_count,AI_action,ai_bet,player_bet,turn_message
    turn_message.set('AI Turn')
    emotion = take_face_emotion.start_get_emotion()
    bet_action_count += 1
    bet_count += 1
    player_bet = (bet_count*5)
    if bet_action_count == 1:
        AI_action = poker_AI.AI_prefrop(ai_hands_index,bet_count,emotion)
        if AI_action == 'bet to ':
            turn_message.set('Player Turn')
            bet_action_count -= 1
            bet_count += 1
...
def end_game(player_act):
    global player_chip_label,player_chip,ai_chip,pot,game_count,ai_qualify_label,player_qualify_label,winner_label,next_game_button,winner,bet_action_count,AI_action,turn_message_lable
    ai_1.delete('ai_1')
    ai_2.delete('ai_2')
...
def next_game():
    global player_qualify_label,ai_qualify_label,next_game_button,winner_label,game_count,trun
...
def start():
    global img_card_ai1,img_card_ai2,img_card_board1,img_card_board2,img_card_board3,img_card_board4,img_card_board5,img_card_player1,img_card_player2,game_count,bet_count,turn_message_label,turn_message
    ...
    turn_message = tkinter.StringVar()
    turn_message.set('Player Turn')
    turn_message_label = tkinter.Label(window,textvariable = turn_message)
    turn_message_label.pack()
